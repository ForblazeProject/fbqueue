...
pub fn run_daemon() {
    ...
    loop {
        ...
        // 0. Cancel logic (manual or walltime)
        ...
        while i < running_jobs.len() {
            let (ref _id, ref mut child, _, _, start_at, walltime) = running_jobs[i];
        ...
        while i < running_jobs.len() {
            if let Ok(Some(status)) = running_jobs[i].1.try_wait() {
                let (id, _, _, _, _start_at, _) = running_jobs.remove(i);
        ...
        // 3. Start new
        let used_capacity: usize = running_jobs.iter().map(|(_, _, cost, _, _, _)| *cost).sum();
        if used_capacity < config.global_capacity {
